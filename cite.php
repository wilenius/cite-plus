<?php
/*
Plugin Name: Cite Plus
Plugin URI: http://github.com/wilenius/cite-plus
Description: Help readers know how to cite your article correctly - use Cite Plus plugin to display a box at the bottom of each page/post with reference information.
Version: 1.2.3
Author: EnigmaWeb & Heikki Wilenius
Author URI: https://www.iki.fi/hw/
*/
...
function cite_shortcode() {
    ...
    // check if coauthors plus is installed before adding coauthors to the citation info
    if (function_exists('get_coauthors') == true) {

        $authorcount = 1;
        $numItems = count(get_coauthors());
        foreach( get_coauthors() as $coauthor ) {
                if ($authorcount === 1) {
                    $authorarray = array($coauthor->last_name, ', ',$coauthor->first_name);
                    $authors = implode($authorarray);
                    $authorcount += 1;
            } else {
                if ($numItems == $authorcount) {
                  $separator = " & ";
                } else {
                      $separator = ", ";
                }
            array_push($authorarray, $separator, $coauthor->display_name);
            $authors = implode($authorarray);
            $authorcount += 1;
            }
        }
    ...
    // Get the DOI address from the custom field
    $doi_address = get_post_meta(get_the_ID(), 'DOI_address', true);
    
    // Check if the DOI address exists and is not empty
    if (!empty($doi_address)) {
        // If it does, use it as the permalink
        $permalink = '<a href="' . $doi_address . '">' . $doi_address . '</a>';
    } else {
        // Otherwise, use the default permalink
        $permalink = '<a href="' . get_permalink() . '">' . get_permalink() . '</a>';
    }
    
    $replace_string = array($authors, get_bloginfo('name'), get_the_title(), displayTodaysDate(), get_the_date('F Y'), $permalink);
